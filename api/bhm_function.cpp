...
// double_matrixr basketmcmc(int_vector dose,int_vector type,double_vector q0,double_vector q1,int_vector y,int_vector n,double sigma_theta,double mu0, double sigma0, double lambda1, double lambda2,int mcmcsize);
...
// \theta_{ij} = logit(p_{ij}) - logit(p_1)
// \theta_{ij}|\mu,\sigma^2 ~ N(\mu,\sigma^2)
// \mu ~ N(\mu_0,\sigma_0^2)
// \sigma^2 ~ Inverse-Gamma(\alpha,\lambda) // \alpha = lambda1, \lambda = \lambda2

// [[Rcpp::export]]
double_matrix bhm_mcmc(int_vector dose,int_vector type,double_vector q0,double_vector q1,int_vector y,int_vector n,double sigma_theta,double mu0, double sigma0, double lambda1, double lambda2,int mcmcsize,int seed) {

  int basket_n, i;

  double mu_pre, sigma_pre, mu_new, sigma_new;
  double theta_cand, temp_a, temp_b, temp_c, temp_mean, temp_sd, temp_lambda1, temp_lambda2;
  ...
  for (int mcmcsim = 0;mcmcsim < mcmcsize;mcmcsim++) {
    ...
    for (i = 0;i < basket_n;i++) {
      ...
      if (unif() < (
          ...
          exp(y[i] * (theta_cand - theta_pre[i]) - n[i] * (log(1/q1[i] - 1 + exp(theta_cand)) - log(1/q1[i] - 1 + exp(theta_pre[i]))))
                      * c_dnorm(theta_cand,mu_pre,sigma_pre) / c_dnorm(theta_pre[i],mu_pre,sigma_pre))
                      ) {
    ...
    temp_b = temp_b / (temp_b + pow(sigma_pre,2));
    temp_mean = temp_b * temp_a / basket_n + (1 - temp_b) * mu0;
    temp_sd = sqrt(pow(sigma_pre,2)*temp_b/basket_n);
    ...
    for (i = 0;i < basket_n;i++) {
      ...
      temp_c += pow(theta_new[i]-mu_new,2);

    }
    temp_lambda1 = basket_n/2+lambda1;
    temp_lambda2 = temp_c/2+lambda2;
    sigma_new=sqrt(rinversegamma(temp_lambda1,temp_lambda2));
    draws_vector.push_back(sigma_new);
