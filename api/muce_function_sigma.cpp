...
double_set muce_mcmc_basket(int seed, int narm, int_vector y, int_vector n, double_vector theta0, double_vector theta1,
	double scale1, double scale3, double mu2, double sigma2, double eta_lambda1, double eta_lambda2,
	double sigma_theta, int mcmcsize);
...
double rinversegamma(double alpha, double lambda) {
	...
	while (1) {
		...
		if (log(unif()) <= ((alpha - 1)*log(lambda*y) + (beta - lambda)*y - log(alpha*tgamma(alpha)))) {
			x = y;
			break;
		}
...
// MCMC code for muce-basket ----
// two intervals, different reference rate (theta0 and theta1) for each basket arm
// =================
// Model: 
// H_0j: p_{j}<p_{0j} versus  H_1j: p_{j}>=p_{0j}, here j indexes indication
// y_{j}|p_{j} ~ Binomial(n_{j},p_{j})    
// p_{j} = logit^{-1}(\theta_{j})
// \theta_{j}| \lambda_{j} ~ f_1(\theta_{j})^{\lambda_{j}=1}*f_2(\theta_{j})^{\lambda_{j}=0}  f_1, f_2: half cauchy, i.e., truncated at (-Inf,\theta_{0j}) and (\theta_{0j},Inf), \theta_{0j}=logit(p_{0j})
// \lambda_{j}=1 if Z_{ij}>0 else \lambda_{j}=0
// Z_{j}|\eta_0,\sigma^2 ~ N(\eta_0,\sigma^2)
// \eta_0 ~ N(\mu, \sigma^2/c0)   G-prior
// \sigma^2 ~ Inverse-Gamma(\eta_lambda1,\eta_lambda2) 

// [[Rcpp::export]]
double_set muce_mcmc_basket_v2(int seed, int narm, int_vector y, int_vector n, double_vector theta0, double_vector theta1, double scale1, double scale3, double mu, double c0, double eta_lambda1, double eta_lambda2, double sigma_theta, int mcmcsize) {

	srand(seed);

	int i;
	double temp_c, temp_b, temp_a, temp_lambda1, temp_lambda2, temp_mean, temp_sd;
	double eta0_new, eta0_pre, sigma_eta_new, sigma_eta_pre, theta_cand;
	double_vector Z_new, Z_pre, theta_pre, theta_new, sigma_eta_new_vec, eta0_new_vec;
	double_matrix eta0_draws, Z_draws, theta_draws, sigma_eta_draw;
	double_set output;
	...
	for (int mcmcsim = 0; mcmcsim < mcmcsize; mcmcsim++) {
		...
		// 1) sample theta_j
		theta_new.clear();
		// temp_c = c_pnorm(-eta0_new / sigma_eta_new); // temp_c = normal_01_cdf(-eta0_new / sigma_eta_new);
		temp_c = c_pnorm(-eta0_pre / sigma_eta_pre);
		for (i = 0; i < narm; i++) {
		...
		for (i = 0; i < narm; i++) {
			if (theta_new[i] < theta0[i]) {
				temp_c = truncated_normal_b_sample(eta0_pre, sigma_eta_pre, 0);
				// if ((temp_c - eta0_pre) / sigma_eta_pre < -2.575829) { temp_c = -2.575829*sigma_eta_pre + eta0_pre;  }
				if (temp_c < -1000) { temp_c = -unif() * 1000; }
				Z_new.push_back(temp_c);
			}
			else {
				temp_c = truncated_normal_a_sample(eta0_pre, sigma_eta_pre, 0);
				if (temp_c > 1000) { temp_c = unif() * 1000; }
				// if ((temp_c - eta0_pre) / sigma_eta_pre > 2.575829) { temp_c = 2.575829*sigma_eta_pre + eta0_pre; }
				Z_new.push_back(temp_c);
			}
		...
		//3) sample sigma_eta
		temp_b = accumulate(Z_new.begin(), Z_new.end(), 0);
		double Z_mean = temp_b / narm; 
		temp_c = 0;
		for (i = 0; i < narm; i++) {
			temp_c += pow(Z_new[i] - Z_mean, 2);
		}
		temp_lambda1 = eta_lambda1 + narm / 2;
		temp_lambda2 = eta_lambda2 + temp_c / 2 + pow(Z_mean - mu, 2) * narm * c0 / (narm + c0) / 2;
		sigma_eta_new = sqrt(rinversegamma(temp_lambda1, temp_lambda2));
		sigma_eta_new_vec.clear();
		for (i = 0; i < narm; i++) {
			sigma_eta_new_vec.push_back(sigma_eta_new);
		}
		...
		temp_c = narm + c0;
		temp_b = accumulate(Z_new.begin(), Z_new.end(), 0);
		temp_mean = (temp_b + mu * c0) / temp_c;
		temp_sd = sqrt(pow(sigma_eta_new, 2) / temp_c);
	...
	output.push_back(sigma_eta_draw);
